...
if uploaded_file is not None:
    ...
    for page_number in range(min(5, len(pdf_document))):
        ...
        # Convert hex color #eeeeee to RGB
        hex_color = '#eeeeee'
        ...
        lower_light_gray = np.array([0, 0, 240])
        upper_light_gray = np.array([180, 40, 255])

        # Create a mask for the light gray color #eeeeee
        mask_eeeeee = cv2.inRange(hsv, lower_light_gray, upper_light_gray)

        # Combine purple with each specified color
        combined_masks = {
            "Networks.": cv2.bitwise_or(mask_eeeeee, mask_purple),
        }

        # Manually define the bounding box coordinates (x_min, y_min, x_max, y_max)
        x_min, y_min, x_max, y_max = 18, 15, 826, 435
        ...
        st.image(cv2.cvtColor(result_with_bbox_page, cv2.COLOR_BGR2RGB), caption=f'Networks - Page {page_number + 1}', use_column_width=True)
